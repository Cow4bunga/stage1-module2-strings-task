...
public class MethodParser {
    ...
    public MethodSignature parseFunction(String signatureString) {
        ...
        String preArgs = tokenizer.nextToken();
        String[] prefixes = preArgs.split(" ");
        if (tokenizer.hasMoreTokens()) {
            String arguments = tokenizer.nextToken();
            String[] args = arguments.split(", ");
            for (String item : args) {
                String[] items = item.split(" ");
                argumentList.add(new MethodSignature.Argument(items[0], items[1]));
            }
        }

        MethodSignature signature = new MethodSignature(prefixes[prefixes.length - 1], argumentList);
